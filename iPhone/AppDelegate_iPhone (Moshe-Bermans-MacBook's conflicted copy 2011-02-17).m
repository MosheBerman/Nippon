...
- (void) newGame:(NSNotification *)notif{
	...
	if(kInGame == YES){
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Start New?", @"prompt the user to restart or go back")
...
- (void) showInstructions:(NSNotification *)notif{
	...
	AboutNipponViewController *instructions = [[AboutNipponViewController alloc] initWithStyle:UITableViewStyleGrouped];
...
- (void) showGameCenterActionSheet: (NSNotification *)notif{
	...
	if (isGameCenterAvailable()) {
		...
		UINavigationController *navController = [[UINavigationController alloc] initWithRootViewController:scoresView];
		[scoresView release];
		
		[self.mainMenu presentModalViewController:navController animated:YES];
...
//
//	TODO: Make categories parameter
//
