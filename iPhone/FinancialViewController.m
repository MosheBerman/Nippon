...
- (id)initWithMode:(NSString *)viewMode andItem:(NSString *)viewItem atPrice:(NSNumber *)viewPrice{
    self = [self initWithNibName:@"FinancialViewController" bundle:nil];
	if(self){
		...
		if(viewPrice != nil){
			[self setPrice:[viewPrice longLongValue]];
		}
		
		//
		//	Configure the amount box
		//
		
		[self.amountBox setKeyboardAppearance:UIKeyboardAppearanceAlert];		
	...
	if([[self mode] isEqualToString:kModeItem]){
		...
		if ([modeSelector selectedSegmentIndex] == 0) {
			...
			if([kCash longLongValue]/self.price > 0){
				[self.infoLabel setText:[NSString stringWithFormat:@"You can afford %lli at %@%i.",[kCash longLongValue]/self.price, kYen,  self.price]];
			}else {
				[self.infoLabel setText:[NSString stringWithFormat:@"You cannot afford %@.", self.title]];
			}
		...
		if ([modeSelector selectedSegmentIndex] == 0) {
			...
			[self.infoLabel setText:[NSString stringWithFormat:@"You have %@%lli.", kYen, [kCash longLongValue]]];
			...
			if([kSavings longLongValue] > 0){
				[self.infoLabel setText:[NSString stringWithFormat:@"You can withdraw up to %@%lli.", kYen, [kSavings longLongValue]]];
			}else {
				[self.infoLabel setText:@"You have no savings."];
			}
		}
		...
		if ([modeSelector selectedSegmentIndex] == 0) {
			...
			if([kDebt longLongValue] > 0){
				[self.infoLabel setText:[NSString stringWithFormat:@"Do you have our %@%lli?", kYen, [kDebt longLongValue]]];
			}else {
				[self.infoLabel setText:[NSString stringWithFormat:@"How can we help you?"]];
			}
            ...
            if([kDebt longLongValue] < kDebtCap){
				[self.infoLabel setText:[NSString stringWithFormat:@"We can lend you up to %@%lli.",kYen, kDebtCap - [kDebt longLongValue]]];
			}else {
				[self.infoLabel setText:@"We can't trust you 'til you pay."];
			}
		}
	}
}
...
- (IBAction) handleMaxButton{
	...
	if([self.mode isEqualToString:kModeItem]){
		...
		if(modeSelector.selectedSegmentIndex == 0){
			...
			NSInteger numberOfEmptySpaces = [kMaxCart longLongValue]- numberOfItemsOnYou;
			...
			//If the player has some room and can afford some of the item
				...
				[self presentBannerWithMessage:@"You don't have enough room and you can't afford any."];
                ...
                [self presentBannerWithMessage:@"You don't have enough room."];
                ...
                [self presentBannerWithMessage:@"You cannot afford any."];
            }
			...
			if ([[kCart objectForKey:self.title] longLongValue] > 0) {
                ...
                [self presentBannerWithMessage:[NSString stringWithFormat:@"You don't have any %@ to sell.", self.title]];
            }			
		}
		...
		if(modeSelector.selectedSegmentIndex == 0){
			...
			if ([kCash longLongValue] > 0) {
				...
				[self presentBannerWithMessage:@"You don't have any money to deposit."];
			}
			...
			if ([kSavings longLongValue] > 0) {
				...
				[self presentBannerWithMessage:@"You don't have any savings to withdraw."];
			}
		}
		...
		if(modeSelector.selectedSegmentIndex == 1){
			...
			if ([kDebt longLongValue] < kDebtCap) {
				...
				[self presentBannerWithMessage:@"Careful, they don't want to lend you more."];
			...
			if([kDebt longLongValue] > 0){
				...
				if ([kCash longLongValue] > [kDebt longLongValue]) {
					...
					if ([kCash longLongValue] > 0) {
						...
						[self presentBannerWithMessage:@"Uh Oh! You don't have money with you."];
				...
				[self presentBannerWithMessage:@"You don't owe anything."];
...
- (void) setPlaceholderText{
	...
	if([self.mode isEqualToString:kModeItem]){
		...
		[self.infoLabel setText:[NSString stringWithFormat:@"It seems that you have %@%lli.", kYen,[kCash longLongValue]]];
...
- (BOOL) performPurchase{
	if ([amountBox.text isEqualToString:@""] || [amountBox.text isEqualToString:@"0"]){
        ...
        [self presentBannerWithMessage:@"You can't buy nothing!"];
		...
		for(NSNumber *itemInCart in [kCart allValues]){
			numberOfItemsOnYou += [itemInCart longLongValue];
		}
		
		numberOfItemsOnYou += [amountBox.text longLongValue];
		
		//
		//	Perform the check
		//
		
		if([amountBox.text longLongValue] * [self price] <= [kCash longLongValue] && numberOfItemsOnYou <= [kMaxCart longLongValue]){
			...
			if([amountBox.text longLongValue] * [self price] > [kCash longLongValue] && numberOfItemsOnYou > [kMaxCart longLongValue]){
                ...
                [self presentBannerWithMessage:[NSString stringWithFormat:@"You cannot buy %@ %@. You don't have enough room and you can't afford it. Try selling some of your other sushi first.", [amountBox text], [self title]]];
                
			}else if ([amountBox.text longLongValue] * [self price] > [kCash longLongValue]) {
                ...
                [self presentBannerWithMessage:[NSString stringWithFormat:@"You cannot buy %@ %@. You can't afford it. Try selling some of your other sushi first.", [amountBox text], [self title]]];
				
			}else if (numberOfItemsOnYou > [kMaxCart longLongValue]) {
                ...
                [self presentBannerWithMessage:[NSString stringWithFormat:@"You cannot buy %@ %@. You don't have enough room. Try selling some of your other sushi first.", [amountBox text], [self title]]];
...
- (BOOL) performSale{
	...
	if ([amountBox.text isEqualToString:@""] || [amountBox.text isEqualToString:@"0"]){
		...
		[self presentBannerWithMessage:@"You can't sell nothing."];
		...
		if ([amountBox.text longLongValue] <= [[kCart objectForKey:self.title] longLongValue]) {
			...
			if (isGameCenterAvailable()) {
				if ([self.title isEqualToString:@"Sushizushi"]) {
						if ([amountBox.text longLongValue] >= 100) {
							[(AppDelegate_iPhone *)[[UIApplication sharedApplication] delegate] reportAchievementIdentifier:kAchievementInheritance percentComplete:100.0];
						}
				}
			}
			...
			[kSettings setObject:[NSNumber numberWithInteger:[kNumberOfItemsSold longLongValue] + [amountBox.text longLongValue]] forKey:@"numberOfItemsSold"];
			...
			if ([[kCart objectForKey:self.title] longLongValue] > 0) {
                ...
                [self presentBannerWithMessage:[NSString stringWithFormat:@"You don't have that many %@ to sell.", self.title]];
				...
				[self presentBannerWithMessage:[NSString stringWithFormat:@"You don't have any %@!", self.title]];
			}
...
- (BOOL) performWithdrawal{
	...
	if ([amountBox.text isEqualToString:@""] || [amountBox.text isEqualToString:@"0"]){
		...
		[self presentBannerWithMessage:[NSString stringWithFormat:@"You can't withdraw %@0...", kYen]];
         
		return NO;
		
	}else{
		
		if ([self.amountBox.text longLongValue] <= [kSavings longLongValue]){
			...
			[self presentBannerWithMessage:[NSString stringWithFormat:@"You don't have %@%@ to withdraw.", kYen, [amountBox text]]];
...
- (BOOL) performDeposit{
	...
	if ([amountBox.text isEqualToString:@""] || [amountBox.text isEqualToString:@"0"]){
		...
		[self presentBannerWithMessage:[NSString stringWithFormat:@"You can't deposit %@0...", kYen]];
		return NO;
	}else {
		
		if ([amountBox.text longLongValue] <= [kCash longLongValue]) {
            ...
            [self presentBannerWithMessage:[NSString stringWithFormat:@"You can't deposit %@%@. You don't have that much!", kYen, [self.amountBox text]]];
...
- (BOOL) performBorrow{
	...
	if ([amountBox.text isEqualToString:@""] || [amountBox.text isEqualToString:@"0"]){
		...
		[self presentBannerWithMessage:[NSString stringWithFormat:@"You can't borrow %@0. It's insulting to even ask for something like that!", kYen]];
		
		return NO;
		
	}else{
		
		if ([kDebt longLongValue] + [self.amountBox.text longLongValue] <= kDebtCap){
			...
			[self presentBannerWithMessage:[NSString stringWithFormat:@"We don't trust you enough to lend you %@%@. Sorry!", kYen, [amountBox text]]];
...
- (BOOL) performPayment{
	...
	if ([amountBox.text isEqualToString:@""] || [amountBox.text isEqualToString:@"0"] || [amountBox.text isEqualToString:@"00000"]){
        ...
        [self presentBannerWithMessage:[NSString stringWithFormat:@"You can't pay %@0.\nThat's insulting!", kYen]];
		...
		if ([self.amountBox.text longLongValue] <= [kDebt longLongValue] && [self.amountBox.text longLongValue] <= [kCash longLongValue]){
            ...
            [self presentBannerWithMessage:[NSString stringWithFormat:@"You don't owe %@%@. Why offer?", kYen, [amountBox text]]];
			
			return NO;
			
		}else if([self.amountBox.text longLongValue] > [kCash longLongValue]){
			
			//
			//	Inform the user that there is not enough cash to repay.
			//
			
			[self presentBannerWithMessage:[NSString stringWithFormat:@"You don't have %@%@.  Do you want to offend them?", kYen, [amountBox text]]];
